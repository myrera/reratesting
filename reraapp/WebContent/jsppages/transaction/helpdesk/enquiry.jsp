...
function myFunction()
{
	var jqElt =$("#my-textarea").val();
var wordmatch = $("#my-textarea").val().match(/[^\s]+\s+/g);
words = wordmatch?wordmatch.length:0;

if (words > wordLimit) {
    var trimmed = $("#my-textarea").val().split(/(?=[^\s]\s+)/, wordLimit).join("");
    var lastChar = $("#my-textarea").val()[trimmed.length];
    $("#my-textarea").val(trimmed + lastChar);
    alert("Maximum allowed 200 words");
}
$('.words-left').text(""+Math.max(wordLimit-words, 0));
}
...
<div> -->
					<div class="form-group has-feedback ">
						...
						<div class="my-container col-sm-9">

							<textarea class="form-control requiredValFld" name="description"  id="my-textarea"></textarea>
								<span class="words-left"></span> words left
					...
					<div class="form-group has-feedback">
					<p><b><font style="color:red;font-size:10px !important;padding-left:40px;">* Note : </font></b><b style="font-size:10px !important;">Projects/Agents filing Related Documents are not allowed to attach through HelpDesk</b></p>
